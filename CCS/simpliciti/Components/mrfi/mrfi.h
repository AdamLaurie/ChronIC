...
#    endif
#    define MRFI_MAX_PAYLOAD_SIZE  (MAX_PAYLOAD + NWK_HDR_SIZE) /* SimpliciTI payload size plus six
                                                                 *byte overhead */
#endif
 ...
 *                                    processing time on peer
 ...
 |   |      |             |         |                                    */
#define   PLATFORM_FACTOR_CONSTANT   (2 + 2 * \
                                      (MAX_HOPS * \
                                       (MRFI_CCA_RETRIES * (8 * MRFI_BACKOFF_PERIOD_USECS) / 1000)))
